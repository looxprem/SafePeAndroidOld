    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        edtxPassCode.addTextChangedListener(new TextWatcher() {
            ...
            public void afterTextChanged(Editable s) {
                if (s.toString().equals(SharedPrefManager.getInstance(activity).getUser().getPasscode())) {
                    clickListener.onPasscodeMatch(true);
                    dismiss();
                } else if (s.length() == 4 && !s.toString().equals(SharedPrefManager.getInstance(activity).getUser().getPasscode())) {
                    error.setText("Invalid Passcode");
                    error.setVisibility(View.VISIBLE);
//                    Toast.makeText(activity, "Invalid Passcode!", Toast.LENGTH_SHORT).show();
